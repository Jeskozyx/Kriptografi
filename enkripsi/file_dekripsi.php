...
// Tentukan sumber file terenkripsi: upload file atau berdasarkan judul dari database
$path_enkripsi = null;

// Mode 1: File diupload langsung (simpan ke folder hasil_enkripsi)
...
// Mode 2: Berdasarkan pilihan dropdown (ID - Judul)
...
// Tutup koneksi DB jika masih terbuka
if (isset($conn) && $conn instanceof mysqli) {
    $conn->close();
}

// Validasi akhir keberadaan file terenkripsi
if (!$path_enkripsi || !file_exists($path_enkripsi)) {
    echo "❌ File terenkripsi tidak ditemukan.";
    exit;
}

// 📄 Buat path hasil dekripsi (nama file sama tapi prefiks 'dec_')
...
// 🐍 Jalankan Python (gunakan path absolut & escapeshellarg agar aman)
...
// gabungkan command (tambahkan 2>&1 biar error ikut terbaca)
